...
export default function Table(props) {
  ...
  useEffect(() => {
    if (props?.data) {
      var data = props?.data;
      if (data?.pagenation_id) setPagenation(data?.pagenation_id);
      // if(data?.column)
          ...
          <table style={tableCSS}>
            ...
            {rows.map((row, index) => (
              <tr
                style={
                  index % 2 === 0
                    ? { backgroundColor: "rgba(217,217,217,1)", ...rowCSS }
                    : { backgroundColor: "white", ...rowCSS }
                }
              >
...
export function NewTable(props) {
  ...
  //   how to resize coulmn in data grid
  ...
  return (
    <div>
      ...
      <Box m="5px 0 0 0" height="85vh" sx={sx}>
        <DataGrid
          rows={props?.rows}

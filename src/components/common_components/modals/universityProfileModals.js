...
import {
  universityContactAddReducer,
  universityMeetingAddReducer,
  universityBasicDetailsUpdateReducer,
  universityProgramAddReducer,
  universityRecentUpdateAddReducer,
  universityMeetingUpdateReducer,
  universityMeetingReducer,
  universityContactUpdateReducer,
  universityRecentUpdateUpdateReducer,
  universityMouContractReducer,
  universityMouContractAddReducer,
  universityMouContractUpdateReducer,
...
import { uploadFile } from '../../../firebase/routes';
...
export function MeetingUpdateUniversityModal(props) {
  ...
  function onSubmit(e) {
    ...
    dispatch(
      universityMeetingUpdateReducer({
        data: { title: data?.title, agenda: data?.agenda },
        id,
      })
    );
  }
    ...
    <div style={style}>
      ...
      <div style={{ marginTop: '3rem' }}>
        <form>
          ...
          <Button
            style={{ marginTop: '1rem', ...buttonCSS }}
            submit={onSubmit}
            buttonText={'Save'}
          />
        </form>
        ...
        <form>
          ...
          >
            ...
            <input
              style={textFeildCSS}
              onChange={(e) =>
                setData({ ...data, forign_name: e.target.value })
              }
              value={data.forign_name}
              type="text"
              placeholder="Final Degree name"
            />
            <input
              style={textFeildCSS}
              onChange={(e) => setData({ ...data, tutionFees: e.target.value })}
...
export function MouContractAddUniversityModal(props) {
  ...
  const reduxData = useSelector(
    (state) => state.universityMouContractSlice.data
  );
...
export function MouContractAddUniversityModal2(props) {
  ...
  function onSubmit(e) {
    ...
    dispatch(
      setMouContractSliceLoading({ loading: true, message: 'Please Wait' })
    );
    ...
    uploadFile(file, `files/${file_name}`).then((fileURL) => {
      ...
      if (d['file'] === '') {
        dispatch(
          setMouContractSliceLoading({
            loading: false,
            message: 'Error in uploading file please try after some time',
          })
        );
        return;
      }
        ...
        <form>
          ...
          >
            ...
            <input
              ...
              type="checkbox"
              onChange={(e) => setData({ ...data, general: !data?.general })}
            />
          </div>
